...
function Dock({
  ...
  return (
    <motion.div
      ...
      className='mx-2 flex max-w-full items-end overflow-x-auto py-2'
    >
      <motion.div
        onMouseMove={({ pageX }) => {
          isHovered.set(1);
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn(
            'absolute -top-6 left-1/2 w-fit whitespace-pre rounded-md border border-blue-500/30 bg-black/80 backdrop-blur-sm px-2 py-0.5 text-xs text-white',
            className
          )}
          role='tooltip'
          style={{ x: '-50%' }}
